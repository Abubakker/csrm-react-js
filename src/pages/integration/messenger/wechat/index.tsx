...
import { WeChatHelp } from 'assets/images';
...
import WeChatForm from 'components/integration/WeChatForm';
import { useGetIntegrationSettingsQuery } from 'store/im-chat-stores/integrationSettingsApi';

// Functional component for WeChat
  ...
  return (
    <div className="-mx-8 -my-10 overflow-hidden min-h-[calc(100vh-64px)] font-NotoSans bg-white p-6">
      <header className="mb-12">
        <h2 className="font-bold text-2xl">{t('we_chat')}</h2>
      </header>
      ...
      <section className="max-w-[493px] space-y-6">
        ...
        <div>
          ...
          <p className="leading-[20px]">{t('integration_we_chat_desc')}</p>
        </div>

        <img src={WeChatHelp} alt="WeChat Help" className="max-w-full"/>
        <UserGuideBtn />
        ...
        <div className="space-y-3">
          ...
          <NoteCard isImportant={true} note={t('we_chat_note1')} />
          <NoteCard isImportant={false} note={t('we_chat_note2')} />
          <NoteCard isImportant={false} note={t('we_chat_note3')} />
        </div>
      ...
      <Modal
        title={
          <h3 className="font-bold">{t('integrations_we_chat_modal_title')}</h3>
        }
        centered
        open={isModalOpen}
        onCancel={closeModal}
      ...
      >
        <p>{t('integrations_we_chat_modal_desc')}</p>
